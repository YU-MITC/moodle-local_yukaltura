...
if ($hassiteconfig) {
    ...
    if ($enableapicalls) {
        ...
        $session = local_yukaltura_login(false, true, '', KALTURA_SESSION_LENGTH, true);
    ...
    // Upload URI.
    $adminsetting = new admin_setting_configtext('upload_uri', get_string('upload_uri', 'local_yukaltura'),
                       get_string('upload_uri_desc', 'local_yukaltura'), '', PARAM_URL);
    $adminsetting->plugin = KALTURA_PLUGIN_NAME;
    $settings->add($adminsetting);

    // Kaltura upload login.
    $adminsetting = new admin_setting_configtext('upload_login', get_string('upload_login', 'local_yukaltura'),
                       get_string('upload_login_desc', 'local_yukaltura'), '', PARAM_TEXT);
    $adminsetting->plugin = KALTURA_PLUGIN_NAME;
    $settings->add($adminsetting);

    // Kaltura upload password.
    $adminsetting = new admin_setting_configpasswordunmask('upload_password',
                       get_string('upload_password', 'local_yukaltura'),
                       get_string('upload_password_desc', 'local_yukaltura'), '');
    $adminsetting->plugin = KALTURA_PLUGIN_NAME;
    $settings->add($adminsetting);

    // Maximum data size (Bytes) of a media entry that a user can upload to Kaltura server.
    $adminsetting = new admin_setting_configtext('maximum_data_size', get_string('maximum_data_size', 'local_yukaltura'),
                       get_string('maximum_data_size_desc', 'local_yukaltura'), KALTURA_DEFAULT_MAXIMUM_DATA_SIZE, PARAM_INT);
    $adminsetting->plugin = KALTURA_PLUGIN_NAME;
    $settings->add($adminsetting);
