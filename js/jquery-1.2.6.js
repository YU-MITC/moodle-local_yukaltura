...
// Will speed up references to undefined, and allows munging its name. undefined.
...
jQuery.extend({
    ...
    removeData: function( elem, name ) {
        ...
        // Otherwise, we want to remove all of the element's data.
            ...
            try {
                ...
                // IE has trouble directly removing the expando but it's ok with using removeAttribute.
    ...
    merge: function( first, second ) {
        ...
        // Also, we need to make sure that the correct elements are being returned (IE returns comment nodes in a '*' query).
        if ( jQuery.browser.msie ) {
            while ( elem = second[ i++ ] ) {
                if ( elem.nodeType != 8 ) {
                    first[ pos++ ] = elem;
...
jQuery.extend({
    ...
    find: function( t, context ) {
        ...
        // Continue while a selector expression exists, and while we're no longer looping upon ourselves.
        while ( t && last != t ) {
            var r = [];
            last = t;
            ...
            // An attempt at speeding up child selectors that point to a specific element tag.
                re = quickChild,
            ...
            // See if there's still an expression, and that we haven't already matched a token.
            if ( t && !foundToken ) {
                ...
                if ( !t.indexOf(",") ) {
                    ...
                    if ( m ) {
                        ...
                        // Otherwise, do a traditional filter check for ID, class, and element selectors.
                        re2 = quickClass;
                        m = re2.exec(t);
                    }
...
jQuery.event = {
    ...
    add: function(elem, types, handler, data) {
         ...
         * this is to prevent a memory leak with non-native event in IE.
         */
        handle.elem = elem;
        ...
        jQuery.each(types.split(/\s+/), function(index, type) {
            ...
            if (!handlers) {
                ...
                /**
                 * Check for a special event handler only use addEventListener/attachEvent
    ...
    fix: function(event) {
        ...
        // Store a copy of the original event object and "clone" to set read-only properties.
...
jQuery.fn.extend({
    ...
    stop: function(clearQueue, gotoEnd){
        ...
        // Dtart the next in the queue if the last step wasn't forced.
        if (!gotoEnd)
            this.dequeue();
...
jQuery.each([ "Height", "Width" ], function(i, name){
    ...
    // Setting innerHeight and innerWidth.
