...
(function (name, context, factory) {
    ...
    var Encoding = {
        ...
        convert: function(data, to, from) {
            ...
            var encodingFrom;
            if (from != null && isString(from) && from.toUpperCase() !== 'AUTO' && !~from.indexOf(',')) {
        ...
        toHiraganaCase: function(data) {
            ...
            while (i < len) {
                ...
                if (c >= 0x30A1 && c <= 0x30F6) {
                    c -= 0x0060;
                } else if (c === 0x30F7) {  // Case: 「ワ゛」 => 「わ」 + 「゛」.
                    results[results.length] = 0x308F;
                    c = 0x309B;
                } else if (c === 0x30FA) { // Case: 「ヲ゛」 => 「を」 + 「゛」.
        ...
        toHankanaCase: function(data) {
            ...
            while (i < len) {
                ...
                } else if (c >= 0x30AB && c <= 0x30C9) { // Case: 「カ」 - 「ド」.
                    results[results.length] = hankanaCase_table[c - 1];
                    results[results.length] = 0xFF9E;
                } else if (c >= 0x30CF && c <= 0x30DD) { // Case: 「ハ」 - 「ポ」.
                    d = c % 3;
        ...
        toZenkanaCase: function(data) {
            ...
            for (i = 0; i < len; i++) {
                ...
                if (c > 0xFF60 && c < 0xFFA0) {
                    ...
                    if (i + 1 < len) {
                        next = data[i + 1];
                        if (next === 0xFF9E && c === 0xFF73) {  // Case : 「ﾞ」 + 「ヴ」.
                            code = 0x30F4;
                            i++;
                        } else if (next === 0xFF9E && c === 0xFF9C) { // Case: 「ﾞ」 + 「ワ゛」.
                            code = 0x30F7;
                            i++;
                        } else if (next === 0xFF9E && c === 0xFF66) { // Case: 「ﾞ」 + 「ｦ゛」.
                            code = 0x30FA;
                            i++;
                        } else if (next === 0xFF9E &&
                                   ((c > 0xFF75 && c < 0xFF85) ||
    ...
    function isUTF8(data) {
        ...
        for (; i < len; i++) {
            ...
            if (b >= 0xC2 && b <= 0xDF) {
                ...
                if (i + 2 >= len || data[i + 1] < 0x80 || data[i + 1] > 0xBF ||
                    data[i + 2] < 0x80 || data[i + 2] > 0xBF) {
                    return false;
                }
                i += 2;
                ...
                if (i + 3 >= len || data[i + 1] < 0x90 || data[i + 1] > 0xBF || data[i + 2] < 0x80 ||
                    data[i + 2] > 0xBF || data[i + 3] < 0x80 || data[i + 3] > 0xBF) {
                    return false;
                }
                i += 3;
            } else if (b >= 0xF1 && b <= 0xF3) {
                if (i + 3 >= len || data[i + 1] < 0x80 || data[i + 1] > 0xBF || data[i + 2] < 0x80 ||
                    data[i + 2] > 0xBF || data[i + 3] < 0x80 || data[i + 3] > 0xBF) {
                    return false;
                }
                i += 3;
            } else if (b === 0xF4) {
                if (i + 3 >= len || data[i + 1] < 0x80 || data[i + 1] > 0x8F || data[i + 2] < 0x80 ||
                    data[i + 2] > 0xBF || data[i + 3] < 0x80 || data[i + 3] > 0xBF) {
                    return false;
                }
                i += 3;
    ...
    var UTF8_TO_JIS_TABLE = {
        ...
        0xEFBFA4:0x7C7C, 0xEFBC87:0x7C7D, 0xEFBC82:0x7C7E, 0xE288A5:0x2142, 0xEFBFA2:0x224C,
        0xE28892:0x1215D
    };
